...
public class FavoriteController {
    ...
    @GetMapping
    public TypeGroupedItemList getFavorite(@AuthenticationPrincipal User user) {
    ...
    public void setFavoriteItem(@AuthenticationPrincipal User user, @RequestBody FavoriteRequestBody body) {
        UserEntity userEntity = userService.findByUsername(user.getUsername());
        // 防止同样的数据(user-item pair)写入两次 => use try-catch
        try {
            favoriteService.setFavoriteItem(userEntity, body.favorite());
        } catch(DbActionExecutionException e) {
            Throwable cause = e.getCause();
            if (cause instanceof DataIntegrityViolationException) { // re-throw
